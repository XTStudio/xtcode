...
export class NetworkInspector {
    ...
    run(context: vs.ExtensionContext, onRight: boolean = false) {
        const panel = vs.window.createWebviewPanel("networkInspector", "Network", onRight === true ? vs.ViewColumn.Two : vs.ViewColumn.One, {
    ...
    getWebviewContents(context: vs.ExtensionContext): string {
        ...
        <body style="margin: 0;">
            <script nonce="${nonce}">
                setTimeout(function(){
                    document.body.innerHTML = '<div id="fullArea" style="position: absolute; left: 0px; right: 0px; height: 100%; opacity: 1;"><iframe src="${redirectSrc.toString()}#http://127.0.0.1:8090/inspector/network.html" style="position: absolute; width: 100%; height: 100%; border: 0px" /></div>'
                }, 100)
            </script>
        </body>
        </html>`
